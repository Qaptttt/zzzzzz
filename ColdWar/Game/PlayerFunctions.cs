    ...
    internal class PlayerFunctions
    {
        ...
        public static string GetPlayerName(int ID)
        {
            ...
            try
            {
                ...
                string name = memoryManager.ReadAsciiString(baseAddress + offset, 15);
                Console.WriteLine($"Player Name (ID {ID}): {name}");
                return name;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to read player name for ID {ID}: {ex.Message}");
